...
/// </param>
public partial class FileResultManager(IStorageProvider storageProvider) : IFileResultManager
{
    /// <summary>
    ///     The storage provider.
    /// </summary>
    ...
    private string GetEpisodeName(FileResult fileResult)
    {
        ...
        var regex = EpisodeNameRegex();
        var episodeNames = (from episode in fileResult.Episodes
            let match = regex.Match(episode.Name)
            where match.Success
            select new { Name = episode.Name[..match.Index].Trim(), Part = match.Groups[1] }).ToList();
