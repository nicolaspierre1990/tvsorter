...
public class ScanManager : IScanManager
{
    ...
    /// </summary>
    public void RefreshFileCounts() => RefreshFileCounts(settings.DestinationDirectories.Select(x => new DirectoryInfoWrap(x)));
    ...
    private TvShow MatchShow(string fileName, int matchIndex, out string showName, bool ignoreShowUpdate)
    {
        ...
        showName = fileName[..(matchIndex - 1)];
    ...
    /// </returns>
    private static IEnumerable<Episode> ProcessEpisode(Match match, TvShow show)
    {
        ...
        if (season.Success && episode.Success)
        {
            ...
            foreach (Capture episodeGroup in match.Groups["E"].Captures.Cast<Capture>())
            {
    ...
    private FileResult ProcessFile(IFileInfo file, bool ignoreShowUpdate)
    {
        ...
        {
            var show = MatchShow(file.Name, match.Index, out string showname, ignoreShowUpdate);
